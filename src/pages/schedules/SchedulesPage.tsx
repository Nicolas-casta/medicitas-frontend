...
export const SchedulesPage = () => {
  ...
  const onSubmit = async (data: ScheduleRequest) => {
    ...
    await createSchedule(selectedDoctor, {
      ...data,
      duracionCitaMinutos: Number(data.duracionCitaMinutos),
    });
    setShowModal(false);
    reset();
    getSchedules(selectedDoctor).then((r) => setSchedules(r.data));
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        {selectedDoctor && (
          <Button onClick={() => setShowModal(true)}>+ Agregar horario</Button>
        )}
      </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid gap-2">
              {schedules.map((s) => (
                <div
                  key={s.id}
                  className="bg-slate-800 border border-slate-700 rounded-lg p-3 flex gap-4 text-sm"
                >
                  ...
                  <span className="text-slate-400">
                    Cada {s.duracionCitaMinutos} min
                  </span>
                </div>
              ))}
              {schedules.length === 0 && (
        ...
        <Modal title="Agregar horario" onClose={() => setShowModal(false)}>
          ...
          >
            ...
            <Input
              label="Hora inicio"
              type="time"
              register={register("horaInicio", { required: true })}
            />
            <Input
              label="Hora fin"
              type="time"
            ...
            <Input
              ...
              type="number"
              register={register("duracionCitaMinutos", {
                required: true,
                min: 10,
              })}
            />
