...
export const DoctorsPage = () => {
    ...
    return (
        <div>
            ...
            <div className="mb-4">
                <select onChange={e => setFilterSpecialty(e.target.value ? Number(e.target.value) : undefined)}
            ...
            <div className="grid gap-3">
                {doctors.map(d => (
                    <div key={d.id} className="bg-slate-800 border border-slate-700 rounded-xl p-4 flex justify-between items-center">
                        <div>
                            <p className="font-medium text-slate-100">{d.nombreCompleto}</p>
                            <p className="text-sm text-slate-400">{d.especialidad} · {d.email}</p>
                            <p className="text-xs text-slate-500">Licencia: {d.licenciaMedica}</p>
                        </div>
                        <div className="flex gap-2">
                            <Button variant="danger" onClick={() => handleDeactivate(d.id)}>Desactivar</Button>
                        </div>
                    </div>
                ))}
            </div>
